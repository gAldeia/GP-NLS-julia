...
# In the GP_NLS paper, they create constants with uniform values between -100 and 100.
# that is, we need an ERC with these values to get initial values of constants
ERCSet = ERC[
    ERC(-100.0, 100.0),
]
...
for df_name in df_names
    ...
    # Let's create the terminal set. In the original paper, the terminals are
    # constants (by ERC) and weighted variables (variable * weight).
    # We are also going to decrease the depth by 1 to compensate the weights
    ...
    for i in 1:30
        ...
        if size(df_filtro[df_filtro.Execution .== i, :], 1) == 0
            ...
            exec_time = @elapsed (bestsol = GP(
                ...
                terminalSet,
                minDepth               = 1,
                maxDepth               = 10 - 2 - 1, # compensating aditional nodes
                maxSize                = 75 - 4,     # compensating aditional nodes
                popSize                = 250,
                gens                   = 400,
                mutationRate           = 0.25,
                elitism                = true,
