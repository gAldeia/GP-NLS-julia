...
# No artigo do método LM, eles criam constantes com valores uniformes entre -100 e 100.
# ou seja, precisamos de uma ERC com esses valores para termos valores iniciais de constantes
ERCSet = ERC[
    ERC(-100.0, 100.0),
]
...
df_results = try
    CSV.File(".results/LsqOptResults.csv") |> DataFrame
catch err
...
for df_name in df_names
    ...
    # Vamos criar o conjunto de terminais. No artigo, os terminais são constantes (pelo ERC)
    # e (variável * peso), então vamos criar os terminais como galhos contendo essa informação
    # (isso é feito utilizando como termianl o WeightedVar).
    # Também vamos diminuir em 1 a profundidade, para compensar esses galhos
    varSet = Union{Var, WeightedVar}[
        WeightedVar(var_name, i) for (i, var_name) in enumerate(names(df_data)[1:end-1])
    ]
    ...
    for i in 1:30
        ...
        if size(df_filtro[df_filtro.Execução .== i, :], 1) == 0
            ...
            exec_time = @elapsed (bestsol = GP(
                ...
                10 - 2 - 1,                        # Profundidade máxima da árvore
                75 - 4,                            # Número máximo de nós permitidos nas árvores
                ...
                true,                              # Fazer o uso do passo de otimização não linear
                true                               # manter os nós de transformação linear na otimização
            ))
            ...
            CSV.write("./results/LsqOptResults.csv", df_results)
